...
RSpec.describe ProductsController do
  describe "GET #index" do
    it "responds successfully with an sucess code " do
       get :index
  ...
  describe "GET #new" do
    ...
    context ".authenticated" do
      admin_login
      it "responds successfully with an sucess status code" do
       get :new
       expect(response).to have_http_status(200)
      end
    end
  ...
  describe "POST #create" do
    context 'ask for login with admin permission' do
      it "redirect_to sign_in page" do
        post :create, params: {prodcut: {name: Faker::Name.name, price: Faker::Number.decimal(2)}}
    ...
    context 'allow user to add prodcut if admin' do
      ...
      it "returns product object" do
        post :create, params: {product: {name: Faker::Name.name, price: Faker::Number.decimal(2)}}
  ...
  describe "PUT #update" do
    ...
    context 'ask for login with admin permission' do
      it "redirect_to sign_in page" do
        put :update, params: {product: {name: Faker::Name.name}, id: @product.id}
        expect(response).to redirect_to "http://test.host/users/sign_in"
      end
    end
    context 'allow user to add prodcut if admin' do
  ...
  describe "GET #destroy" do
    login_admin
    before("GET #destroy") { @product = FactoryBot.create(:product) }
    it "responds successfully with an HTTP 200 status code" do
